...
def img_transform(img, post_rot, post_tran,
    ...
    post_tran -= torch.Tensor(crop[:2])  # [0,-48] 裁剪左上角坐标
    if flip:
...
normalize_img = torchvision.transforms.Compose((
                torchvision.transforms.ToTensor(), # # 将PIL Image或numpy.ndarray转换为tensor, 并除255归一化到[0,1]之间
                torchvision.transforms.Normalize(mean=[0.485, 0.456, 0.406], # 作用是将数据转换为标准正太分布，使模型更容易收敛
                                 std=[0.229, 0.224, 0.225]), # 是从 ImageNet 数据集的数百万张图片中随机抽样计算得到的参数
))
...
def get_batch_iou(preds, binimgs):
    ...
    with torch.no_grad(): # 不记录梯度
        ...
        intersect = (pred & tgt).sum().float().item() # 交集
        union = (pred | tgt).sum().float().item() # 并集
